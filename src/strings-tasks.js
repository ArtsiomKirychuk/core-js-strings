...
function removeFirstOccurrences(str, value) {
  const startMatchIndex = str.indexOf(value);
  return startMatchIndex >= 0
    ? str.slice(0, startMatchIndex) +
        str.slice(startMatchIndex + value.length, str.length)
    : str;
}
...
function removeLastOccurrences(str, value) {
  const lastMatchIndex = str.lastIndexOf(value);
  return lastMatchIndex >= 0
    ? str.slice(0, lastMatchIndex) +
        str.slice(lastMatchIndex + value.length, str.length)
    : str;
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; ) {
    if (vowels.has(str[i].toLocaleLowerCase())) count += 1;
    i += 1;
  }
...
function isPalindrome(str) {
  const processedString = str.replace(/[^\w]/g, '').toLowerCase();
  return reverseString(processedString).includes(processedString);
}
